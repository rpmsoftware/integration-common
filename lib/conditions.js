...
const {
    getEager, toBoolean, validateString, toArray, normalizeInteger, isEmpty,
...
const OPERATORS = {
    ...
    notEmpty: {
        init: function (conf) {
            let { trim, operands, objects } = conf;
            trim = toBoolean(trim) || undefined;
            operands = toArray(operands).map(o => ({ property: validatePropertyConfig(o) }));
            objects = toBoolean(objects) || undefined;
            assert(operands.length > 0);
            return { operands, trim, objects };
        },
        process: function (data) {
            const { operands, trim, objects } = this;
...
function init(conf) {
    ...
    Array.isArray(conf) && (conf = conf.length > 1 ? { operator: 'and', operands: conf } : conf[0]);
    typeof conf === 'string' && (conf = { operator: conf });
