...
const BASE_CRM_URL = 'https://api.hubapi.com/crm/v3/';
...
class HubSpotAPI {
    ...
    async getProperties(objectType, includeArchived) {
        ...
        const url = new URL(BASE_CRM_URL + `properties/${objectType}`);
    ...
    demandObject(objectType, id, properties) {
        ...
        id = +id;
        assert(id > 0);
        const url = new URL(BASE_CRM_URL + `objects/${objectType}/${id}`);
    ...
    async getObjects(objectType, properties) {
        validateString(objectType);
        const url = new URL(BASE_CRM_URL + `objects/${objectType}`);
    ...
    getOwners() {
        return this._fetch(BASE_CRM_URL + 'owners').then(({ results }) => results);
    }

    getOwner(id) {
        id=+id;
        assert(id>0);
        return this._fetch(BASE_CRM_URL + `owners/${id}`);
    }
    ...
    updateObject(objectType, id, properties) {
        ...
        return this._fetch(
            BASE_CRM_URL + `objects/${objectType}/${id}`,
    ...
    createObject(objectType, properties) {
        ...
        assert(!isEmpty(properties));
        return this._fetch(
            BASE_CRM_URL + `objects/${objectType}`,
    ...
    async getAssociations(fromObjectType, fromObjectID, toObjectType) {
        ...
        const url = new URL(BASE_CRM_URL + `objects/${fromObjectType}/${fromObjectID}/associations/${toObjectType}`);
    ...
    getAssociationLabels(fromObjectType, toObjectType) {
        validateString(fromObjectType);
        validateString(toObjectType);
        return this._fetch(BASE_CRM_URL + `associations/${fromObjectType}/${toObjectType}/labels`);
    }
    ...
    createAssociation(fromObjectType, fromObjectID, toObjectType, toObjectID, typeID, category) {
        ...
        assert(toObjectID > 0);
        return this._fetch(
            BASE_CRM_URL + `objects/${fromObjectType}/${fromObjectID}/associations/${toObjectType}/${toObjectID}`,
