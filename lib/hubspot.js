...
class HubSpotAPI {
    ...
    constructor({ accessToken, cacheTTL }) {
        cacheTTL = +cacheTTL;
    ...
    async _getPropertiesCached(objectType) {
    ...
    async _extendObject(object, objectType) {
        ...
        const [propertyDefinitions, owners] = await Promise.all([
            this._getPropertiesCached(objectType),
            this._getOwnersCached()
        ]);
        for (const propName in objectProperties) {
            ...
            const { options, type, referencedObjectType } = getEager(propertyDefinitions, propName);
            if (type !== TYPE_ENUMERATION) {
                continue;
            }
            if (referencedObjectType === REFERENCE_TYPE_OWNER) {
                const { id, email, type, firstName, lastName } = owners.demand(({ id }) => id === propValue);
                objectProperties[propName] = { id, email, type, firstName, lastName };
            } else {
                const def = options.find(({ value }) => value === propValue);
                if (def) {
                    const { label, value } = def;
                    objectProperties[propName] = { label, value };
                }
            }
        }
