...
const OBJECT_TYPE_ALIASES = {
    ...
    goal_targets: ObjectTypes.GoalTarget,
    leads: ObjectTypes.Lead,
    quotes: ObjectTypes.Quote,
    tickets: ObjectTypes.Ticket
};

Object.keys(ObjectTypes).forEach(k => {
    const type = ObjectTypes[k];
    OBJECT_TYPE_ALIASES[type] = type;
});
...
const normalizeObjectType = type => getEager(OBJECT_TYPE_ALIASES, type.toLowerCase());
...
class HubSpotAPI {
    ...
    async _extendObject(object, objectType) {
        ...
        for (const propName in objectProperties) {
            const propValue = objectProperties[propName];
            ...
            if (isEmptyValue(propValue)) {
                multiple && (objectProperties[propName] = []);
                continue;
            }

            let result = multiple ? propValue.split(ValueDelimiter) : [propValue];

            if (referencedObjectType === REFERENCE_TYPE_OWNER) {
                result = result.map(pv => {
                    const { id, email, type, firstName, lastName } =
                        owners.demand(({ id }) => id === pv);
                    return { id, email, type, firstName, lastName };
                });
            } else {
                const empty = options.length < 1;
                result = result.map(propValue => {
                    if (empty) {
    ...
    async getOwners() {
        const url = new URL(BASE_CRM_URL_V3 + 'owners');
        url.searchParams.append('limit', 500);
        let active = this._fetch(url.toString());
        url.searchParams.append('archived', true);
        let archived = this._fetch(url.toString());
        active = await active;
        assert(!active.paging);
        archived = await archived;
        assert(!archived.paging);
        return active.results.concat(archived.results);
    }
    ...
    createAssociation(fromObjectType, fromObjectID, toObjectType, toObjectID, typeID, category) {
        category = AssociationCategories[category] || category;
