...
const HEADERS_TIMEOUT = 15 * 60 * 1000;
const NA = 'n/a';
...
function API(url, key) {
    ...
    this.agent = new Agent({ headersTimeout: HEADERS_TIMEOUT });
    ...
    this.validateFieldPresence = true;
}
...
API.prototype._validateResultFields = function ({ Fields: inputFields }, { Fields: resultFields }) {
    if (!this.validateFieldPresence || !inputFields) {
        return;
    }
    assert(Array.isArray(inputFields));
    assert(Array.isArray(resultFields));
    if (inputFields.length < 1) {
        return;
    }
    inputFields.forEach(({ Field: inField }) =>
        resultFields.find(({ Field }) => Field === inField) || throwFieldNotFound(inField)
    );
};
...
API.prototype.editForm = async function (processNameOrID, formNumberOrID, fields, properties, control) {
    ...
    let result = await this.request('ProcFormEdit', body);
    this._validateResultFields(properties, result.Form);
    result = this._extendForm(result);
...
API.prototype.getFields = async function (processID) {
    ...
    FieldGroups.forEach(fieldGroup => {
        ...
        if (Type === ObjectType.FormReference) {
            ...
            /* Workaround for missing Statuses. Yes, it happens */
            const ids = [];
            Values.forEach(v => {
                const sl = StatusLevels.find(({ Text }) => Text === v);
                sl && ids.push(sl.ID);
            });
            Values = ids;
        }
...
API.prototype.addFormFile = function (formID, fileName, fileData, folderID, description, shared) {
    ...
    if (typeof formID === 'object') {
        ...
        AlternateID = formID.AlternateID || undefined;
...
API.prototype.createForm = async function (processOrId, fields, properties, fireWebEvent) {
    ...
    const result = await this.request('ProcFormAdd', properties);
    this._validateResultFields(properties.Form, result.Form);
    return this._extendForm(result);
};
...
API.prototype.getCustomer = function () {
    return this.demandCustomer.apply(this, arguments).catch(e => {
        if (e.Message !== errors.MSG_CUSTOMER_NOT_FOUND) {
            throw e;
        }
    });
};
...
API.prototype.createCustomer = async function (data) {
    ...
    const result = await this.request('CustomerAdd', { Customer: data })
    this._validateResultFields(data, result);
    return this._normalizeCustomer(result);
};

API.prototype.editCustomer = async function (nameOrID, data) {
    ...
    const result = await this.request('CustomerEdit', { Customer: customer });
    this._validateResultFields(customer, result);
    return this._normalizeCustomer(result);
};
...
API.prototype.editCustomerContact = function (customerID, contactID, data, primary) {
    ...
    primary === undefined || (body.IsPrimary = toBoolean(primary));
    return this.request('CustomerContactEdit', body).then(result => result.Contact);
};
...
API.prototype.createAgency = async function (data, fireWebEvent) {
    if (typeof data !== 'object') {
        data = { Agency: data };
    }
    const result = await this.request('AgencyAdd', { Agency: data, WebhookEvaluate: !!fireWebEvent });
    this._validateResultFields(data, result);
    return this._normalizeAgency(result);
};

API.prototype.editAgency = async function (id, data, fireWebEvent) {
    ...
    const result = await this.request('AgencyEdit', { Agency: data, WebhookEvaluate: !!fireWebEvent });
    this._validateResultFields(data, result);
    return this._normalizeAgency(result);
};
...
API.prototype.createRep = async function (agencyID, data) {
    ...
    const result = await this.request('RepAdd', { Rep: data });
    this._validateResultFields(data, result);
    return this._normalizeRep(result);
};

API.prototype.editRep = async function (repID, data) {
    ...
    data = Object.assign({}, data);
    data.RepID = repID;
    const result = await this.request('RepEdit', { Rep: data });
    this._validateResultFields(data, result);
    return this._normalizeRep(result);
};
...
API.prototype.createSupplier = async function (data) {
    if (typeof data !== 'object') {
        data = { Supplier: data };
    }
    const result = await this.request('SupplierAdd', { Supplier: data });
    this._validateResultFields(data, result);
    return this._normalizeSupplier(result);
};

API.prototype.editSupplier = async function (id, data) {
    if (typeof id === 'object') {
        data = id;
        id = data.SupplierID;
    }
    ...
    data.SupplierID = id;
    const result = await this.request('SupplierEdit', { Supplier: data });
    this._validateResultFields(data, result);
    return this._normalizeSupplier(result);
};
...
API.prototype.getCommAccounts = function (run, excludeZeroTotals) {
    if (this.validateParameters) {
        run = run ? validateString(run) : 'all';
        excludeZeroTotals === undefined || (excludeZeroTotals = toBoolean(excludeZeroTotals));
    }
    return this.request('CommAccounts', { Run: run, ExcludeZeroTotals: excludeZeroTotals });
};

API.prototype.getCommReps = function (run) {
    if (this.validateParameters) {
        run = run ? validateString(run) : 'all';
    }
    return this.request('CommReps', { Run: run });
};
