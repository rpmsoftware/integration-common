...
const arrayPrototypeExtensions = {
    ...
    aggregate: function (aggrProp, reducer, groupProps) {
        const result = this.group(aggrProp, groupProps);
        result.forEach(e => e[aggrProp] = e[aggrProp].reduce(reducer, undefined));
        return result;
    },

    aggregateMerge: function (reducer, groupProps) {
        const result = this.group(PROP_AGGREGATE, groupProps);
        result.forEach(e => {
    ...
    group: function (aggrProp, groupProps) {
        ...
        this.forEach(e => {
            ...
            let key = groupProps.map(p => {
                ...
                return isEmptyValue(v) ? EMPTY_STRING : v;
            });
            key = `[${key.join('][')}]`
            let grp = result[key];
            if (!grp) {
...
exports.createParallelRunner = (parallelRequests, perMilliseconds) => {
    ...
    perMilliseconds > 0 || (perMilliseconds = 0);
    ...
    const shift = () => {
        ...
        if (slots.length < parallelRequests) {
            ...
            slot = slots.find(({ available, lastUsed }) => available && now - lastUsed > perMilliseconds);
        }
        if (!slot) {
            return setTimeout(shift, MS_WAIT);
        }
        const cb = queue.shift();
        slot.lastUsed = now;
        delete slot.available;
        cb().finally(() => slot.available = true);
    };
...
const validateFetchResponse = exports.validateFetchResponse = async response => {
    ...
    throwError(statusText || status, FETCH_ERROR, { status, statusText, response, headers });
};
...
exports.createCaselessGetter = object => {
    const obj = {};
    for (const key in object) {
        obj[key.toLocaleLowerCase()] = object[key];
    }
    return name => obj[name.toLocaleLowerCase()];
};

exports.isResponseJSON = ({ headers }) => {
