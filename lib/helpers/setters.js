...
add('List', function (config, data) {
    const { srcField, demand, options, defaultValue, skipEmpty } = config;
    let value = getDeepValue(data, srcField);
    isEmptyValue(value) && (value = defaultValue);
    if (isEmptyValue(value)) {
        return skipEmpty ? undefined : getEmpty();
    }
...
}, async function ({ demand, defaultValue, skipEmpty }, rpmField) {
    demand = toBoolean(demand) || undefined;
    ...
    options = options
        ...
        .map(({ Text, ID }) => ({ Text, ID }));
    return { options, demand, defaultValue, skipEmpty };
});
...
async function initField(conf, rpmField) {
    ...
    let { setter, condition, srcField, normalize, regexp, skipEmpty } = conf;
    ...
    const isNotTable = !isTableField(rpmField);
    if (condition && isNotTable) {
        conf.condition = initCondition(condition);
    }
    conf.skipEmpty = toBoolean(skipEmpty) || undefined;
    return conf;
}
...
async function setField(conf, data, form) {
    ...
    try {
        ...
        if (conf.skipEmpty && isEmptyValue(result)) {
            return;
        }
    } catch (error) {
