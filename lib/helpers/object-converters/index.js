...
const OBJECT_CONVERTERS = {
    ...
    group: {
        init: function ({ group, children, array, dstProperty }) {
            validateString(children);
            dstProperty = dstProperty ? validateString(dstProperty) : (array ? validateString(array) : undefined);
            array = array ? validatePropertyConfig(array) : undefined;
            group = toArray(group);
            assert(group.length > 0);
            group.forEach(validateString);
            return { group, children, array, dstProperty };
        },

        convert: function ({ group, children, array, dstProperty }, data) {
            if (!array) {
                assert(Array.isArray(data));
                return data.group(children, group);
            }
            toArray(data).forEach(e => {
                const a = getDeepValue(e, array);
                if (!a) {
                    return;
                }
                assert(Array.isArray(a));
                e[dstProperty] = a.group(children, group)
            });
            return data;
        }
    },
    ...
    if: {
        init: async function ({ condition, convert, break: breakIfTrue }) {
            condition = initCondition(condition);
            convert = await init.call(this, convert);
            breakIfTrue = toBoolean(breakIfTrue) || undefined;
            return { convert, condition, break: breakIfTrue };
        },
        convert: async function ({ convert: convertConf, condition, break: breakIfTrue }, obj) {
            let flag;
            for (const e of toArray(obj)) {
                const isTrue = processCondition(condition, e);
                flag = flag || isTrue;
                isTrue && await convert.call(this, convertConf, e);
            }
            if (breakIfTrue && flag) {
                throw BREAK_TAG;
            }
    ...
    toKeyValue: {
        init: function ({ srcProperty, dstProperty, keyProperty }) {
            srcProperty = validatePropertyConfig(srcProperty);
            keyProperty = validatePropertyConfig(keyProperty);
            dstProperty = validateString(dstProperty);
            return { srcProperty, dstProperty, keyProperty };
        },

        convert: function ({ srcProperty, dstProperty, keyProperty }, obj) {
            for (const e of toArray(obj)) {
                const array = getDeepValue(e, srcProperty);
                typeof array === 'object' && (
                    e[dstProperty] = (Array.isArray(array) ? array : Object.values(array)).toObject(keyProperty)
                );
            }
            return obj;
        }
    ...
    macro: {
        init: async function ({ id, convert }) {
            validateString(id);
            const macros = this[MACRO_DEFINITIONS] || (this[MACRO_DEFINITIONS] = {});
            assert(!macros[id]);
            macros[id] = await init.call(this, convert);
        },
    ...
    use: {
        init: async function ({ id }) {
            validateString(id);
            const convert = this[MACRO_DEFINITIONS][id];
            assert(convert, `Cannot find macro "${id}"`);
            return { convert };
        },

        convert: function ({ convert: cfg }, obj) {
            return convert.call(this, cfg, obj);
        }
    }

};

const MACRO_DEFINITIONS = Symbol();
